...
def parse_args(kwargs):
    ...
    kwargs['model_path'] = kwargs['model_path'] if 'model_path' in kwargs else False
...
def split_dataset(args, dataset, k):
    ...
    Y = dataset.targets
#     Y = {x: dataset[x].class_to_idx for x in ['train', 'val']}
...
def get_dataset(args, transform, split='train'):
    ...
    elif args.dataset == 'software_defect':
        data_dir = os.path.join(DATASET_PATH, 'ant1.6_poi3.0')
        dataset = torchvision.datasets.ImageFolder(os.path.join(data_dir, 'train'), transform=transform)
        
        if split == 'val':
            dataset = torchvision.datasets.ImageFolder(os.path.join(data_dir, split), transform=transform)

#         if split == 'train':
#             cur_path = os.path.dirname( os.path.abspath( '__file__' ) )
#             print(next(os.walk(folder_path))[0], ': ' ,len(next(os.walk(folder_path))[2]))
         
#         dataset = {x: torchvision.datasets.ImageFolder(os.path.join(data_dir, x), transform=transform) for x in ['train', 'val']}
        
#         if split == 'train':
#             dataset = torchvision.datasets.ImageFolder(os.path.join(data_dir, 'train'), transform=transform)
#         elif split == 'val':
#             dataset = torchvision.datasets.ImageFolder(os.path.join(data_dir, 'val'), transform=transform)
...
#         assert args.dataset == 'cifar10' # TODO: FastAutoAugment for Imagenet
        ...
        transform =  transforms.Compose([
            ...
            transforms.RandomChoice([transforms.RandomGrayscale(p=0.3), 
...
def train_step(args, model, optimizer, scheduler, criterion, batch, step, writer, device=None):
    ...
    #        writer.add_image('train/input_image', concat_image_features(images[j], output[j]), global_step=step)
